package com.virtualightning.webview.common;
...
//            conn.setRequestMethod(request.getMethod());
            ...
            if ("POST".equals(request.getMethod())) {
                ...
                LogUtils.INSTANCE.log(new String(os.toByteArray()));
                builder.post(RequestBody.create(null, os.toByteArray()));
                os.close();
            }
            else builder.get();
            ...
            // Read input
            String charset = response.header("content-encoding") != null ? response.header("content-encoding")
                    : Charset.defaultCharset().displayName();
            String mime = response.header("content-type");
            byte[] pageContents = Utils.consumeInputStream(response.body().byteStream());
            response.close();
